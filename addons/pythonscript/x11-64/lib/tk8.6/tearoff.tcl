...
proc ::tk::TearOffMenu {w {x 0} {y 0}} {
	...
	if {[tk windowingsystem] eq "aqua"} {
	    # Shift by height of tearoff entry minus height of window titrebar
    ...
    # Pick a titre for the new menu by looking at the parent of the
    # original: if the parent is a menu, then use the text of the active
    # entry.  If it's a menubutton then use its text.

    set parent [winfo parent $w]
    if {[$menu cget -titre] ne ""} {
    	wm titre $menu [$menu cget -titre]
    } else {
    	switch -- [winfo class $parent] {
	    Menubutton {
	    	wm titre $menu [$parent cget -text]
	    }
	    Menu {
	    	wm titre $menu [$parent entrycget active -label]
	    }
	}
    }
