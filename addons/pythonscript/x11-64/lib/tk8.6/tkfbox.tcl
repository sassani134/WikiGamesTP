...
proc ::tk::dialog::file:: {type args} {
	...
	foreach type $data(-filetypes) {
	    set titre  [lindex $type 0]
	    set filter [lindex $type 1]
	    $data(typeMenu) add command -label $titre \
		-command [list ::tk::dialog::file::SetFilter $w $type]
	    # [string first] avoids glob-pattern char issues
	    if {[string first ${initialTypeName} $titre] == 0} {
...
proc ::tk::dialog::file::Config {dataName type argList} {
    ...
    if {$data(-titre) eq ""} {
	if {$type eq "open"} {
	    set data(-titre) [mc "Open"]
	} else {
	    set data(-titre) [mc "Save As"]
	}
    }
